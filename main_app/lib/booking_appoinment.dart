...
class _BookingAppointmentState extends State<BookingAppointment> {
  ...
  void _fetchDoctors() async {
    FirebaseFirestore.instance.collection('Doctors_LTA').get().then((querySnapshot) {
      setState(() {
  ...
  void _fetchAvailableDates() async {
    ...
    FirebaseFirestore.instance.collection('Doctors_LTA').doc(_selectedDoctor).get().then((docSnapshot) {
      setState(() {
        _availability = {};
  ...
  if (_formKey.currentState!.validate()) {
    try {
      await FirebaseFirestore.instance.collection('Hospital_Appointments').add({
        'patientName': _nameController.text,  // Patient Name
        'phoneNumber': _phoneController.text, // Phone Number 
        'address': _addressController.text,   // Address
        'doctor': _selectedDoctor,            // Selected Doctor
        'date': _dateController.text,         // Selected Date
        'time': _timeController.text,         // Selected Time
        'reason': _reasonController.text,     // Reason for Visit
  ...
  Widget _buildTextField(String label, TextEditingController controller, {TextInputType keyboardType = TextInputType.text, int maxLines = 1, bool readOnly = false, VoidCallback? onTap}) {
    return Padding(
  ...
  Widget _buildDoctorDropdown() {
    return DropdownButtonFormField<String>(
      ...
      items: _doctorsList.map((doctor) => DropdownMenuItem(value: doctor, child: Text(doctor))).toList(),
      onChanged: (value) {
        setState(() {
